...
(async () => {
  ...
  try {
...

    {
      ...
      assert.equal(await tab('会话管理').count(), 1);
      ...
      for (const client of ['claude-desktop', 'zcode', 'opencode']) {
        await open('client=' + client + '&app-only');
      ...
      assert.equal(await button('手动备份').count(), 0);
      await open('client=pi&no-plugin');
      assert.ok(await button('安装 Pi 插件').isEnabled());

      await open('client=deepseek-harness&running');
      ...
      await open('client=deepseek-harness&running&harness-mode=headless');
      assert.equal(await button('重启 Web').count(), 0);
      await open('client=deepseek-harness&running&fail-restart');
      await button('重启 Web').click();
    ...
    for (const width of [1280, 540]) for (const theme of ['light', 'dark']) {
      ...
      for (const subpage of ['Basic configuration', 'Configuration management']) {
        ...
        await page.screenshot({ path: path.join(screenshots, `${width}-${theme}-full-${subpage.startsWith('Basic') ? 'core' : 'management'}.png`), fullPage: true });
      }
    }
    assert.deepEqual(errors, []);
    console.log('PASS: management navigation, preserved drafts, shared runtime actions, all desktop restarts, Pi, Harness, disabled states, keyboard, responsive themes');
