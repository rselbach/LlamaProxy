...
describe('Windows 便携更新清单', () => {
  test('URL、大小和哈希与实际上传资产一致', async () => {
    ...
    try {
      ...
      for (const [arch, contents] of Object.entries(payloads)) {
        await writeFile(
          join(root, `LlamaProxy-v1.2.3-Windows-${arch}.zip`),
          contents,
        );
      }
      ...
      const manifest = await generatePortableUpdateManifest({
        directory: root,
        output,
        repository: 'rselbach/LlamaProxy',
        gitcodeRepository: 'mirror-owner/LlamaProxy',
        tag: 'v1.2.3',
      ...
      for (const arch of ['amd64', 'aarch64'] as const) {
        ...
        expect(asset.url).toBe(
          `https://github.com/rselbach/LlamaProxy/releases/download/v1.2.3/LlamaProxy-v1.2.3-Windows-${arch}.zip`,
        );
        expect(asset.fallbackUrls).toEqual([
          `https://api.gitcode.com/api/v5/repos/mirror-owner/LlamaProxy/releases/v1.2.3/attach_files/LlamaProxy-v1.2.3-Windows-${arch}.zip/download`,
        ]);
  ...
  test('缺少任一架构资产时拒绝生成清单', async () => {
    ...
    try {
      await writeFile(
        join(root, 'LlamaProxy-v1.2.3-Windows-amd64.zip'),
        'amd64',
      );
  ...
  test.each([
    ...
    try {
      for (const arch of ['amd64', 'aarch64']) {
        await writeFile(
          join(root, `LlamaProxy-v1.2.3-${display}-${arch}.${suffix}`),
