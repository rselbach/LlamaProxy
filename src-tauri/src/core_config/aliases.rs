...
pub(crate) async fn fetch_active_oauth_alias_channels(
    ...
    let response = client
        ...
        .await
        .map_err(|error| {
            format_management_request_error("Failed to read OAuth credential sources", &error)
        })?;
    ...
    let files = payload
        ...
        .ok_or_else(|| "The OAuth credential source response lacks a files array".to_string())?;
    Ok(files
        .iter()
        .filter(|file| {
            !file
...
pub(crate) fn validate_thinking_alias_effort(value: &str) -> Result<String, String> {
    ...
    if effort.chars().all(|character| character.is_ascii_digit()) {
        return Err(
            "Fixed reasoning aliases do not support numeric budgets; enter a reasoning effort name"
                .to_string(),
        );
    }
    if effort.len() > 64
    ...
    {
        return Err("Invalid reasoning effort format; only letters, numbers, hyphens, underscores, and dots are allowed".to_string());
    }
    Ok(effort)
}
...
pub(crate) async fn fetch_management_config_yaml(config: &GuiConfigFile) -> Result<String, String> {
    ...
    let response = client
        ...
        .await
        .map_err(|error| {
            format_management_request_error("Failed to read core YAML configuration", &error)
        })?;
...
pub(crate) async fn put_management_config_yaml(
    ...
    let response = client
        ...
        .await
        .map_err(|error| {
            format_management_request_error("Failed to save core YAML configuration", &error)
        })?;
...
pub(crate) async fn put_management_oauth_model_aliases(
    ...
    let response = client
        ...
        .await
        .map_err(|error| {
            format_management_request_error("Failed to save OAuth model aliases", &error)
        })?;
    read_management_value(response).await.map(|_| ())
}
    ...
    let oauth_model_aliases = if current_oauth == updated_oauth {
        ...
        Some(
            serde_json::to_value(updated_oauth)
                .map_err(|error| format!("Failed to serialize OAuth model aliases: {error}"))?,
        )
    };
    ...
    let (models, mut model) = match source {
        ClaudeDesktopAliasSource::Provider {
            ...
            let models = yaml_mapping_value_mut(root, section)
                ...
                .and_then(serde_norway::Value::as_sequence_mut)
                .ok_or("Model source configuration changed; refresh and retry")?;
            (models, model)
        }
...
pub(crate) async fn fetch_oauth_channel_model_definitions(
    ...
    let response = client
        ...
        .await
        .map_err(|error| {
            format_management_request_error(
                &format!("Failed to read {channel} OAuth model definitions"),
                &error,
            )
        })?;
    let payload = read_management_value(response).await?;
...
pub(crate) fn insert_thinking_effort_params(
    ...
    match source.kind.as_str() {
        ...
        _ => match source.protocol.as_str() {
            ...
            protocol => {
                return Err(format!("Forcing a reasoning effort override for {protocol} sources is not supported yet"));
            }
        },
    }
    Ok(())
...
pub(crate) fn append_config_thinking_alias(
    ...
    let provider = providers
        ...
        .and_then(serde_norway::Value::as_mapping_mut)
        .ok_or_else(|| "The model provider changed; refresh and retry".to_string())?;
    ...
    let source = models
        .get(model_index)
        .cloned()
        .ok_or_else(|| "The original model changed; refresh and retry".to_string())?;
    let (_, current_model, _) = configured_model_identity(&source)
        .ok_or_else(|| "Invalid original model configuration".to_string())?;
    if !current_model.eq_ignore_ascii_case(expected_model) {
        return Err("The original model changed; refresh and retry".to_string());
    }
...
pub(crate) fn remove_thinking_alias_from_yaml_for_channel(
    ...
    if !removed {
        return Err(format!(
            "Model alias {alias} does not exist; refresh and retry"
        ));
    }
...
pub(crate) fn open_external_url_inner(app: &tauri::AppHandle, url: &str) -> Result<(), String> {
    ...
    app.opener()
        ...
        .map_err(|err| format!("Failed to open the browser: {err}"))
}
