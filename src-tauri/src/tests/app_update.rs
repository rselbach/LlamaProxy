...
fn macos_update_helper_stays_inside_the_signed_app_bundle() {
    let current_exe = PathBuf::from("/Applications/LlamaProxy.app/Contents/MacOS/LlamaProxy");
...
fn portable_update_test_asset(version: &str, arch: &str) -> PortableUpdateAsset {
    ...
    let name = format!("LlamaProxy-v{version}-{display}-{arch}.{suffix}");
    PortableUpdateAsset {
...
fn portable_update_test_legacy_asset(version: &str, arch: &str) -> PortableUpdateAsset {
    let name = format!("LlamaProxy-update-v{version}-Windows-{arch}.zip");
    PortableUpdateAsset {
        url: format!("{APP_RELEASE_DOWNLOAD_PREFIX}v{version}/{name}"),
        fallback_urls: Vec::new(),
        sha256: "ab".repeat(32),
...
fn portable_update_test_manifest(version: &str) -> PortableUpdateManifest {
    ...
    PortableUpdateManifest {
        ...
        release_url: format!("https://github.com/rselbach/LlamaProxy/releases/tag/v{version}"),
        assets: [
            (
...
fn portable_update_manifest_requires_both_matching_github_assets() {
    ...
    upstream_release.release_url =
        "https://github.com/router-for-me/EasyCLIProxyAPI/releases/tag/v1.2.3".to_string();
    ...
    foreign_host
            ...
            .unwrap()
            .url = "https://github.com.example.invalid/rselbach/LlamaProxy/releases/download/v1.2.3/update.zip".to_string();
...
fn portable_update_asset_accepts_only_the_configured_gitcode_fallback() {
    ...
    let (_, display, suffix) = portable_update_asset_platform().unwrap();
    let filename = format!("LlamaProxy-v1.2.3-{display}-amd64.{suffix}");
    asset.fallback_urls = vec![gitcode_release_attachment_url(
        "mirror-owner/LlamaProxy",
        "v1.2.3",
        &filename,
    )];
...
fn sha256_file_hashes_exact_portable_asset_bytes() {
    ...
    fs::write(&asset, b"LlamaProxy portable update").unwrap();

    assert_eq!(
        sha256_file(&asset).unwrap(),
        "0199d014e4596329877e205679c9550a4cf0a0781e11400ddc8b1a3c75623c97"
    );
...
fn portable_update_zip_accepts_the_complete_release_package() {
    ...
    let manifest = br#"{"schemaVersion":1,"application":"LlamaProxy","version":"1.2.3","platform":"windows","arch":"amd64","autoUpdate":true}"#;
    write_portable_update_zip(
        &valid_zip,
        &[
            (
                "LlamaProxy-v1.2.3-Windows-amd64/LlamaProxy.exe",
                b"new executable",
                None,
            ),
            (
                "LlamaProxy-v1.2.3-Windows-amd64/portable-app.json",
                manifest,
                None,
            ),
            (
                "LlamaProxy-v1.2.3-Windows-amd64/core-version.txt",
                b"7.2.109\n",
                None,
            ),
            (
                "LlamaProxy-v1.2.3-Windows-amd64/cpa-core/CLIProxyAPI_7.2.109_windows_amd64.zip",
                b"core archive",
                None,
            ),
        ],
    ...
    write_portable_update_zip(
        &traversal_zip,
        &[
            ("../LlamaProxy.exe", b"malicious", None),
            (
                "LlamaProxy-v1.2.3-Windows-amd64/portable-app.json",
                manifest,
                None,
...
fn portable_update_tar_gz_accepts_the_complete_linux_release() {
    ...
    let package_root = "LlamaProxy-v1.2.3-Linux-amd64";
    let manifest = br#"{"schemaVersion":1,"application":"LlamaProxy","version":"1.2.3","platform":"linux","arch":"amd64","autoUpdate":true}"#;
    for (path, contents, mode) in [
        (
            format!("{package_root}/LlamaProxy"),
            b"binary".as_slice(),
            0o755,
        ),
        (
...
fn macos_update_descriptor_is_confined_to_the_app_and_temp_directories() {
    ...
    let work_dir = root.join("LlamaProxy-update-1.2.3-1-1");
    let current_app = root.join("Applications").join("LlamaProxy.app");
    let executable_relative_path = PathBuf::from("Contents/MacOS/llamaproxy");
