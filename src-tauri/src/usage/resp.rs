...
impl UsageSubscription {
    pub(super) async fn connect(port: u16, management_key: &str) -> Result<Self, String> {
        ...
        let stream = timeout(CONNECT_TIMEOUT, TcpStream::connect(&address))
            ...
            .map_err(|error| {
                format!("Failed to connect to the CPA usage subscription {address}: {error}")
            })?;
        ...
        match subscription.read_frame().await? {
            ...
            RespValue::Error(error) => {
                return Err(format!(
                    "CPA usage subscription authentication failed: {error}"
                ))
            }
            value => {
                return Err(format!(
                    "Invalid CPA usage subscription authentication response: {}",
                    value.kind()
                ))
            }
        }
    ...
    async fn read_frame(&mut self) -> Result<RespValue, String> {
        loop {
            ...
            let read = timeout(IO_TIMEOUT, self.stream.read(&mut chunk))
                ...
                .map_err(|error| {
                    format!("Failed to read the CPA usage subscription response: {error}")
                })?;
            if read == 0 {
...
fn parse_resp_frame(input: &[u8], offset: usize, depth: usize) -> Result<ParseResult, String> {
    if depth > MAX_NESTING_DEPTH {
        return Err("CPA usage subscription response is nested too deeply".to_string());
    }
    ...
    match prefix {
        b'+' | b'-' | b':' => {
            ...
            let text = String::from_utf8(line.to_vec())
                .map_err(|_| "CPA usage subscription text response is not UTF-8".to_string())?;
            let value =
                match prefix {
                    b'+' => RespValue::Simple(text),
                    b'-' => RespValue::Error(text),
                    _ => RespValue::Integer(text.parse::<i64>().map_err(|_| {
                        "Invalid CPA usage subscription integer response".to_string()
                    })?),
                };
        ...
        b'$' => {
            ...
            let length = usize::try_from(length)
                .map_err(|_| "Invalid CPA usage subscription string length".to_string())?;
        ...
        b'*' => {
            ...
            let length = usize::try_from(length)
                .map_err(|_| "Invalid CPA usage subscription array length".to_string())?;
...
fn parse_resp_length(value: &[u8]) -> Result<i64, String> {
    std::str::from_utf8(value)
        .map_err(|_| "CPA usage subscription length is not UTF-8".to_string())?
        .parse::<i64>()
